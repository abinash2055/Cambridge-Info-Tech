            ...
            <tbody>
              @foreach ($posts as $post)
                ...
                <tr>
                  ...
                  <td><a href="{{route('employer.show',['employer'=>$post->company])}}">{{substr($post->company->title,0,14)}}..</a></td>
                  <td>{{$post->vacancy_count}}</td>
                  <td>{{date('d/m/Y',$post->deadlineTimestamp())}}, {{date('d',$post->remainingDays()) }} days</td>
                  <td><form action="{{route('account.savedJob.destroy',['id'=>$post])}}" method="POST">
                    @csrf
                    @method("delete")
                ...
                @else
                <tr>
                  <td>You have not applied any jobs.</td>
                  <td></td>
                  <td></td>
                  <td></td>
                  <td></td>
